...
export default tseslint.config(
	{
		...
		rules: {
			...
			'vue/html-indent': ['error', 'tab'], // Enforce tabs
			...
			'@stylistic/js/operator-linebreak': ['error', 'before'], // requires linebreaks to be placed before the operator.
			'@stylistic/js/quotes': ['error', 'single', { avoidEscape: true }], // Enforce single quotes for strings, allowing double quotes only when necessary to avoid escaping.
			'@stylistic/ts/quotes': ['error', 'single', { avoidEscape: true }], // Extend js/quotes for typescript classes.
			'@stylistic/js/indent': ['error', 'tab'], // Enforce tabs.
			'@stylistic/ts/indent': ['error', 'tab'], // Extend js/indent for typescript classes.
			'@stylistic/js/new-parens': 'error', //  Enforce parentheses when invoking a constructor with no arguments.
			'@stylistic/js/no-trailing-spaces': 'error', // Disallows trailing whitespaces.
			'@stylistic/js/eol-last': 'error', // Require newline at the end of files.
			'@stylistic/js/max-len': ['error', { code: 120, tabWidth: 4 }], // Enforces a maximum line length to increase code readability and maintainability.
			'@stylistic/js/block-spacing': 'error', // Enforces consistent spacing inside an open block token and the next token on the same line.
			'@stylistic/ts/block-spacing': 'error', // Extends js/block-spacing for typescript classes.
			...
			'no-relative-import-paths/no-relative-import-paths': ['error', { allowSameFolder: false, rootDir: 'src', prefix: '@' }], // Enforces absolute import paths.
		},
		settings: {
